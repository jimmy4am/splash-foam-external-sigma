...
const UpsellTemplate3 = ({ info, nextStep, sessionData }: Props) => {
  const [productNum, setProductNum] = useState<number>(1);
  const [subscribe, setSubscribe] = useState(false);
  const [showInfo, setShowInfo] = useState(false);

  const subPrices: Prices = {
    1: Number(info.stickyprice1).toFixed(0).toString(),
    2: Number(info.stickyprice2).toFixed(0).toString(),
    3: Number(info.stickyprice3).toFixed(0).toString(),
    4: Number(info.stickyprice4).toFixed(0).toString(),
  };
  const subIds: Prices = {
    1: info.stickyid1,
    2: info.stickyid2,
  ...
  const subNames: Prices = {
    1: info.stickyname1,
    2: info.stickyname2,
    3: info.stickyname3 || "",
    4: info.stickyname4 || "",
  };
  const subBillings: Prices = {
    1: info.stickybilling1,
    2: info.stickybilling2,
    3: info.stickybilling3 || "",
    4: info.stickybilling4 || "",
  };
  const subOfferIds: Prices = {
    1: info.stickyoffer1,
    2: info.stickyoffer2,
    3: info.stickyoffer3 || "",
    4: info.stickyoffer4 || "",
  ...
  const acceptOffer = async () => {
    const upsell = {
      offerName: subscribe ? subNames[productNum] : oneTimeNames[productNum],
      offerPrice: parseFloat(
        subscribe ? subPrices[productNum] : oneTimePrices[productNum]
      ),
      offerId: subscribe ? subIds[productNum] : oneTimeIds[productNum],
      offerBilling: subscribe
        ? subBillings[productNum]
        : oneTimeBillings[productNum],
      offerOfferId: subscribe
        ? subOfferIds[productNum]
        : oneTimeOfferIds[productNum],
    };
          ...
          <div className="flex w-full mt-2 flex-col items-center px-4">
            ...
            <h1 className="text-[16px] pt-2 md:pt-0 md:text-[23px] font-[700] italic text-[#111] text-center">
              ...
              <span className="text-[#44b374]">
                {info.title2}{" "}
                <PriceDisplay
                  priceUSD={parseFloat(info.title6)}
              ...
              <div className="flex w-full mt-2 md:mt-6 relative ">
                ...
                <div className="flex w-full z-10 p-2 md:p-4">
                  ...
                  <div className="flex w-full md:w-1/2 flex-col items-center md:*:items-start justify-center pr-2 text-center md:text-left">
                    ...
                    />
                    <p className="text-[16px] md:text-[20px]">{info.title3}</p>
                    <h2 className="text-[20px] md:text-[30px] font-bold uppercase py-2">
                      {info.product}!
                    </h2>
                    <div className="flex w-full">
            ...
            <div className="flex flex-col w-full items-center md:px-4 mt-4">
              {!info.stickyCta && (
                <>
                  <button
                    className="bg-[#29af5c] text-[23px] uppercase  text-white py-2 px-4 md:py-4 flex  green-text-shadow  items-center hover:transform  font-bold md:w-[90%] justify-center max-w-[775px]  hover:bg-[#0ebf52] rounded-md"
                    onClick={acceptOffer}
                  >
                    {info.ctaText1}
                  </button>
            ...
            {showInfo && (
              <div className="flex w-full text-[10px] mt-4 flex-col items-center text-center max-w-[500px] border-[1px] border-blue-900 p-4 mb-4">
                <h5 className="font-bold">
                  Save {info.savings1} with auto-deliveries
                </h5>
                <p className="text-justify">{info.text2}</p>
              </div>
            )}
          </div>
        </div>
